...
public class BasePerformanceSupportIT {
    ...
    protected static final TestLogger logger =  new TestLogger(BasePerformanceSupportIT.class);
    ...
    protected static String getBaseUrl() {
        ...
        String port = TestPropertiesUtil.getSystemOrTestProp(TEST_DCP_PORT_PROP_NAME);

        if (port != null) {
            String host = TestPropertiesUtil
                    .getSystemOrTestProp(TEST_DCP_HOST_PROP_NAME, "127.0.0.1");
            String baseUrl = String.format("http://%s:%s", host, port);
            return baseUrl;
        }

        return TestPropertiesUtil.getSystemOrTestProp(TEST_DCP_URL_PROP_NAME);
    }
    ...
    protected static ComputeState addHost(ComputeState computeState) throws Exception {
        ...
        SimpleHttpsClient.execute(SimpleHttpsClient.HttpMethod.PATCH,
                getBaseUrl()
                        + HostContainerListDataCollection.DEFAULT_HOST_CONTAINER_LIST_DATA_COLLECTION_LINK,
                Utils.toJson(body));
        SimpleHttpsClient.HttpResponse httpResponse = SimpleHttpsClient.execute(
                SimpleHttpsClient.HttpMethod.PUT,
    ...
    protected static String sendRequest(SimpleHttpsClient.HttpMethod method, String link, String body)
            throws Exception {
        SimpleHttpsClient.HttpResponse httpResponse = SimpleHttpsClient.execute(method,
                getBaseUrl() + buildServiceUri(link), body);
        if (httpResponse.responseBody == null && SimpleHttpsClient.HttpMethod.GET == method) {
    ...
    protected static void waitForTaskToComplete(final String documentSelfLink) throws Exception {
        ...
        for (int i = 0; i < TASK_CHANGE_WAIT_POLLING_RETRY_COUNT; i++) {
            ...
            body = sendRequest(SimpleHttpsClient.HttpMethod.GET, documentSelfLink, null);
    ...
    protected static void waitForStatusCode(URI uri, Map<String, String> headers,
        ...
        while (System.currentTimeMillis() <= toFinish) {
            try {
                SimpleHttpsClient.HttpResponse httpResponse = SimpleHttpsClient.execute(
                        SimpleHttpsClient.HttpMethod.GET,
