...
public class HostPortProfileService extends StatefulService {
    ...
    public static class HostPortProfileState extends ResourceState {
        ...
        /**
         * Default Docker host port start range + 1000.
         * Starting a 1000 ports off the default range will help avoid a race condition when
         * a container that is marked to expose all ports provisions at the same time
         * as container that exposes specific port.
         * In this case Docker might allocates the same port as vRA.
         */
        public static final long PROFILE_RANGE_START_PORT = Long.getLong(
                "dcp.management.host.port.profile.range.start.port", 33768);

        public static final long PROFILE_RANGE_END_PORT = Long.getLong(
                "dcp.management.host.port.profile.range.end.port", 61000);
    ...
    public void handleCreate(Operation start) {
        ...
        if (state.startPort == 0) {
            state.startPort = HostPortProfileState.PROFILE_RANGE_START_PORT;
        }
        if (state.endPort == 0) {
