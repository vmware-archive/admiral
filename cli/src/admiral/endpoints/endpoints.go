...
func GetResourcePoolLinksOfVsphereEndpoints() map[Endpoint]string {
	el := &EndpointList{}
	_, err := el.FetchEndpoints()
	if err != nil {
		return nil
	}
	endpointsToRplinks := make(map[Endpoint]string)
	for _, endpoint := range el.Documents {
		if endpoint.EndpointType != VsphereEndpoint {
			continue
		}
		endpointsToRplinks[endpoint] = endpoint.ResourcePoolLink
	}
	return endpointsToRplinks
}
