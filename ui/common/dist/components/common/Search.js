...
module.exports = (function () {
    function Search(properties, changeCallback) {
        ...
        var optionsMatcher = function (q, cb) {
            ...
            if (q && properties.suggestionProperties) {
                ...
                suggestions.push(q);
                for (var i = 0; i < properties.suggestionProperties.length; i++) {
        ...
        this.$el.on('typeahead:beforeopen', function (e) {
            var ta = $(e.target).data('tt-typeahead');
            if (!ta.input.getQuery()) {
                e.preventDefault();
            }
        });
        this.$el.on('click', 'a.query-occurrence', function (e) {
            e.preventDefault();
            var oldValue = getQueryOccurrence.call(_this);
            var newValue = $(e.target).data('occurrence').name;
            if (oldValue !== newValue) {
                setQueryOccurrence.call(_this, newValue);
                changeCallback();
            }
        });
    }
    Search.prototype.getQueryOptions = function () {
        var tokens = this.$input.tokenfield('getTokens');
        var queryOptions = {};
        if (this.$occurrenceSelector) {
            queryOptions[searchConstants.SEARCH_OCCURRENCE.PARAM] = getQueryOccurrence.call(this);
        }
        for (var i = 0; i < tokens.length; i++) {
            var token = tokens[i].value;
            var type = token.substring(0, token.indexOf(SEPARATOR));
            var value = void 0;
    ...
    Search.prototype.setQueryOptions = function (queryOptions) {
        ...
        if (queryOptions) {
            for (var key in queryOptions) {
                ...
                for (var i = 0; i < currentOptions.length; i++) {
                    ...
                    else if (key === searchConstants.SEARCH_OCCURRENCE.PARAM) {
        ...
        for (var i = 0; i < newTokensStrings.length; i++) {
            newTokens.push({
